...
namespace Microsoft.CodeAnalysis.Editing
{
    internal class DirectivesCustomOrder
    {
        ...
        private PooledDictionary<string, int> ProcessImportDirectivesCustomOrder(ReadOnlyMemory<char> importDirectivesCustomOrder)
        {
            ...
            if (importDirectivesCustomOrderSpan.Length == 0)
                goto addPattern;

            var i = 0;
            for (; i < importDirectivesCustomOrderSpan.Length; i++)
            {
            ...
            // Check if the last character was a SEPARATOR
            // if so, we already parsed the whole input
            // otherwise, there must still be a pattern group to parse
            // -1 because i must be equal to importDirectivesCustomOrderSpan.Length - 1
            if (importDirectivesCustomOrderSpan[i - 1] != SEPARATOR)
            {
                if (!ParsePatternGroup(doubleJokerSpan))
                {
...
addPattern:
            ...
            bool ParsePatternGroup(ReadOnlySpan<char> doubleJokerSpan)
            {
